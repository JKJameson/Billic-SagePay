<?php
class SagePay {
	public $settings = array(
		'description' => 'Accept payments via SagePay.',
	);
	function payment_features() {
		return '<img src=" data:image/false;base64,">';
	}
	function payment_button($params) {
		global $billic, $db;
		$id = get_config('sagepay_vendorid');
		$id = trim($id);
		if (empty($id)) {
			return false;
		}
		return 'Pay Now';
	}
	function payment_page($params) {
		global $billic, $db;
		$html = '';
		if (get_config('sagepay_vendorid') == '') {
			return $html;
		}
		if ($billic->user['verified'] == 0 && get_config('sagepay_require_verification') == 1) {
			return 'verify';
		} else {
			$txcode = date('Ymd') . '-' . str_replace('.', '', microtime(true)) . '-' . $params['invoice']['id'] . '-' . uniqid('');
			$fields = array(
				'VPSProtocol' => '3.00',
				'TxType' => 'PAYMENT',
				'Vendor' => get_config('sagepay_vendorid') ,
				'VendorTxCode' => $txcode,
				'Amount' => $params['charge'],
				'Currency' => get_config('billic_currency_code') ,
				'Description' => 'Invoice #' . $params['invoice']['id'],
				'NotificationURL' => 'http' . (get_config('billic_ssl') == 1 ? 's' : '') . '://' . get_config('billic_domain') . '/Gateway/SagePay/',
				'RedirectionURL' => 'http' . (get_config('billic_ssl') == 1 ? 's' : '') . '://' . get_config('billic_domain') . '/User/Invoices/ID/' . $params['invoice']['id'] . '/Status/Complete/',
				'BillingSurname' => $billic->user['lastname'],
				'BillingFirstnames' => $billic->user['firstname'],
				'BillingAddress1' => $billic->user['address1'],
				'BillingAddress2' => $billic->user['address2'],
				'BillingCity' => $billic->user['city'],
				'BillingPostCode' => $billic->user['postcode'],
				'BillingCountry' => $billic->user['country'],
				'DeliverySurname' => $billic->user['lastname'],
				'DeliveryFirstnames' => $billic->user['firstname'],
				'DeliveryAddress1' => $billic->user['address1'],
				'DeliveryAddress2' => $billic->user['address2'],
				'DeliveryCity' => $billic->user['city'],
				'DeliveryPostCode' => $billic->user['postcode'],
				'DeliveryCountry' => $billic->user['country'],
			);
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, 'https://' . get_config('sagepay_platform') . '.sagepay.com/gateway/service/vspserver-register.vsp');
			curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($fields));
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
			curl_setopt($ch, CURLOPT_HEADER, 0);
			curl_setopt($ch, CURLOPT_FAILONERROR, true);
			$data = curl_exec($ch);
			if ($data === false) {
				echo 'Curl error: ' . curl_error($ch);
				exit;
			}
			curl_close($ch);
			$reply = array();
			$lines = explode(PHP_EOL, $data);
			foreach ($lines as $line) {
				$line = explode('=', $line);
				$key = trim($line[0]);
				unset($line[0]);
				$reply[$key] = trim(implode('=', $line));
			}
			if ($reply['Status'] != 'OK') {
				echo 'SagePay error: ' . $reply['StatusDetail'];
				exit;
			}
			$params['invoice']['tmp'].= PHP_EOL . json_encode(array(
				'SecurityKey' => $reply['SecurityKey'],
				'amount' => $params['charge']
			)); // 'VPSTxId'=>$reply['VPSTxId']
			$params['invoice']['tmp'] = trim($params['invoice']['tmp']);
			$db->q('UPDATE `invoices` SET `tmp` = ? WHERE `id` = ?', $params['invoice']['tmp'], $params['invoice']['id']);
			echo 'Redirecting you to SagePay... <a href="' . $reply['NextURL'] . '">click here</a> if you are not automatically redirected.';
			header('Location: ' . $reply['NextURL']);
		}
		return $html;
	}
	function payment_callback() {
		global $billic, $db;
		//{"VPSProtocol":"3.00","TxType":"PAYMENT","VendorTxCode":"20141210-14182271486179-41-54886dcc96f1b","VPSTxId":"{2BF74527-4D8A-D6C4-7AA2-2C65A4401F35}","Status":"OK","StatusDetail":"0000 : The Authorisation was Successful.","TxAuthNo":"986044","AVSCV2":"SECURITY CODE MATCH ONLY","AddressResult":"NOTMATCHED","PostCodeResult":"NOTMATCHED","CV2Result":"MATCHED","GiftAid":"0","3DSecureStatus":"NOTCHECKED","CardType":"AMEX","Last4Digits":"0004","VPSSignature":"2EC0EBC827FD3400F274395AF428927D","DeclineCode":"00","ExpiryDate":"0215","BankAuthCode":"99972"}
		if ($_POST['TxType'] != 'PAYMENT') {
			return 'Unhandled TxType';
		}
		$invoiceid = explode('-', $_POST['VendorTxCode']);
		if (count($invoiceid) != 4) {
			return 'Invalid VendorTxCode';
		}
		$invoiceid = $invoiceid[2];
		$invoice = $db->q('SELECT * FROM `invoices` WHERE `id` = ?', $invoiceid);
		$invoice = $invoice[0];
		if (empty($invoice)) {
			return 'Invalid Invoice ID';
		}
		if ($_POST['Status'] != 'OK') {
			return 'Status not OK (' . $_POST['StatusDetail'] . ')';
		}
		$amount = 0;
		$md5check = false;
		$sessions = explode(PHP_EOL, $invoice['tmp']);
		foreach ($sessions as $session) {
			$session = json_decode($session, true);
			$md5 = strtoupper(md5($_POST['VPSTxId'] . $_POST['VendorTxCode'] . $_POST['Status'] . $_POST['TxAuthNo'] . strtolower(get_config('sagepay_vendorid')) . $_POST['AVSCV2'] . $session['SecurityKey'] . $_POST['AddressResult'] . $_POST['PostCodeResult'] . $_POST['CV2Result'] . $_POST['GiftAid'] . $_POST['3DSecureStatus'] . $_POST['CAVV'] . $_POST['AddressStatus'] . $_POST['PayerStatus'] . $_POST['CardType'] . $_POST['Last4Digits'] . $_POST['DeclineCode'] . $_POST['ExpiryDate'] . $_POST['FraudResponse'] . $_POST['BankAuthCode']));
			if ($md5 == $_POST['VPSSignature']) {
				$md5check = true;
				$amount = $session['amount'];
				break;
			}
		}
		if ($md5check === false) {
			return 'VPSSignature does not match checksum';
		}
		echo 'OK';
		$billic->module('Invoices');
		return $billic->modules['Invoices']->addpayment(array(
			'gateway' => 'SagePay',
			'invoiceid' => $invoice['id'],
			'amount' => $amount,
			'currency' => get_config('billic_currency_code') ,
			'transactionid' => $_POST['TxAuthNo'],
		));
	}
	function settings($array) {
		global $billic, $db;
		if (empty($_POST['update'])) {
			echo '<form method="POST"><input type="hidden" name="billic_ajax_module" value="SagePay"><table class="table table-striped">';
			echo '<tr><th>Setting</th><th>Value</th></tr>';
			echo '<tr><td>SagePay Vendor ID</td><td><input type="text" class="form-control" name="sagepay_vendorid" value="' . safe(get_config('sagepay_vendorid')) . '"></td></tr>';
			echo '<tr><td>SagePay Platform</td><td><select class="form-control" name="sagepay_platform"><option value="live"' . (get_config('sagepay_platform') == 'live' ? ' selected' : '') . '> Live</option><option value="test"' . (get_config('sagepay_platform') == 'test' ? ' selected' : '') . '> Test</option></select></td></tr>';
			echo '<tr><td colspan="2" align="center"><input type="submit" class="btn btn-default" name="update" value="Update &raquo;"></td></tr>';
			echo '</table></form>';
		} else {
			if (empty($billic->errors)) {
				set_config('sagepay_vendorid', $_POST['sagepay_vendorid']);
				set_config('sagepay_platform', $_POST['sagepay_platform']);
				$billic->status = 'updated';
			}
		}
	}
}
